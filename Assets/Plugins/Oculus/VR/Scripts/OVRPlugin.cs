...
public static partial class OVRPlugin
{
        ...
        get
        {
            ...
            if (_version == null)
            {
                ...
                if (_version > _versionZero && _version < OVRP_1_3_0.version)
                    throw new PlatformNotSupportedException("Oculus Utilities version " + wrapperVersion +
                                                            " is too new for OVRPlugin version " + _version.ToString() +
    ...
    public struct CameraDeviceIntrinsicsParameters
    {
        ...
        double
            disto0; /* Distortion factor : [ k1, k2, p1, p2, k3 ]. Radial (k1,k2,k3) and Tangential (p1,p2) distortion.*/
    ...
    public static bool EnqueueSubmitLayer(bool onTop, bool headLocked, bool noDepthBufferTesting, IntPtr leftTexture,
        ...
        bool overrideTextureRectMatrix = false, TextureRectMatrixf textureRectMatrix = default(TextureRectMatrixf),
        bool overridePerLayerColorScaleAndOffset = false, Vector4 colorScale = default(Vector4),
        Vector4 colorOffset = default(Vector4),
        bool expensiveSuperSample = false, bool bicubic = false, bool efficientSuperSample = false,
        bool efficientSharpen = false, bool expensiveSharpen = false, bool hidden = false)
    {
#if OVRPLUGIN_UNSUPPORTED_PLATFORM
		return false;
#else
        ...
        if (version >= OVRP_1_6_0.version)
        {
            ...
            if (version >= OVRP_1_34_0.version && layerId != -1)
                return OVRP_1_34_0.ovrp_EnqueueSubmitLayer2(flags, leftTexture, rightTexture, layerId, frameIndex,
                           ref pose, ref scale, layerIndex,
                           overrideTextureRectMatrix ? Bool.True : Bool.False, ref textureRectMatrix,
                           overridePerLayerColorScaleAndOffset ? Bool.True : Bool.False, ref colorScale,
                           ref colorOffset) ==
                       Result.Success;
        ...
        float frequency, float amplitude)
    {
        ...
        {
            Result result =
                OVRP_1_78_0.ovrp_SetControllerLocalizedVibration(controllerMask, hapticsLocationMask, frequency,
                    amplitude);
    ...
    public static bool OverrideExternalCameraFov(int cameraId, bool useOverriddenFov, Fovf fov)
    {
        ...
        if (version >= OVRP_1_44_0.version)
        {
            bool retValue = true;
            Result result =
                OVRP_1_44_0.ovrp_OverrideExternalCameraFov(cameraId, useOverriddenFov ? Bool.True : Bool.False,
                    ref fov);
            if (result != Result.Success)
            {
                retValue = false;
    ...
    public static bool IsInsightPassthroughSupported()
    {
        ...
        if (version >= OVRP_1_71_0.version)
        {
            ...
            Debug.LogError(
                "Unable to determine whether passthrough is supported. Try calling IsInsightPassthroughSupported() while the XR plug-in is initialized. Failed with reason: " +
                result);
            return false;
        }
        else
    ...
    public static PassthroughCapabilityFlags GetPassthroughCapabilityFlags()
    {
        ...
        if (version >= OVRP_1_78_0.version)
        {
            ...
            Debug.LogError(
                "Unable to retrieve passthrough capability flags. Try calling GetInsightPassthroughCapabilityFlags() while the XR plug-in is initialized. Failed with reason: " +
                result);
        }
        else
        {
            Debug.LogWarning(
                "ovrp_GetPassthroughCapabilityFlags() not yet supported by OVRPlugin. Result of GetInsightPassthroughCapabilityFlags() is not accurate.");
        }
    ...
    public class Media
    {
		...
		return false;
            ...
            {
                Result res =
                    OVRP_1_49_0.ovrp_Media_SetHeadsetControllerPose(headsetPose, leftControllerPose,
                        rightControllerPose);
...
// Virtual keyboard calls
    ...
    public static bool QuerySpaces(SpaceQueryInfo queryInfo, out UInt64 requestId)
    {
        ...
        if (version >= OVRP_1_72_0.version)
        {
            ...
            else if (queryInfo.FilterType == SpaceQueryFilterType.Components)
            {
                ...
                {
                    Debug.LogError(
                        "QuerySpaces attempted to query more components than the maximum number supported: " +
                        SpaceFilterInfoComponentsMaxSize);
                    return false;
                }
            }
    ...
    [Obsolete(nameof(LocateSpace) +
              " unconditionally returns a pose, even if the underlying OpenXR function fails. Instead, use TryLocateSpace, which indicates failure.")]
    ...
    private struct SceneCaptureRequestInternal
    {
        ...
        [MarshalAs(UnmanagedType.LPStr)] public string request;
    }
    ...
    [Obsolete(
        "This method allocates managed arrays. Use GetSpaceBoundary2D(UInt64, Allocator) to avoid managed allocations.")]
    ...
    private static class OVRP_1_1_0
    {
        ...
        {
            return Marshal.PtrToStringAnsi(OVRP_1_1_0._ovrp_GetVersion());
        }
        ...
        {
            return Marshal.PtrToStringAnsi(OVRP_1_1_0._ovrp_GetNativeSDKVersion());
        }
        ...
        {
            return Marshal.PtrToStringAnsi(OVRP_1_1_0._ovrp_GetSystemProductName());
        }
        ...
        {
            return Marshal.PtrToStringAnsi(OVRP_1_1_0._ovrp_GetAppLatencyTimings());
        }
    ...
    private static class OVRP_1_15_0
    {
        ...
        public static extern Result
            ovrp_GetLayerTexturePtr(int layerId, int stage, Eye eyeId, ref IntPtr textureHandle);
    ...
    private static class OVRP_1_72_0
    {
        ...
        [DllImport(pluginName, CallingConvention = CallingConvention.Cdecl)]
        public static extern Result
            ovrp_GetSpaceRoomLayout(ref UInt64 space, ref RoomLayoutInternal roomLayoutInternal);
